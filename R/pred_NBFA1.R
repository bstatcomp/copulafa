#' Predict from NBFA1 model
#'
#' This function predicts based on the estimation of the NBFA1
#' model \code{train_NBFA1}.
#'
#' @export
#' @param tr_fa An object resulting from \code{train_NBFA1}.
...
pred_NBFA1 <- function (tr_fa, gp_test, test, logl = T) {
  ...
  if (anyNA(pr_mu)) {
    ...
    tmp_prob            <- pr_probs[wh_md, -1]
    tmp_mu              <- pr_mu[wh_md, -1]
    tmp_size            <- pr_size[wh_md, -1]
    pr_probs[ind, -1]   <- tmp_prob
    pr_mu[ind, -1]      <- tmp_mu
